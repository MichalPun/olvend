...
create table if not exists public.telemetry_sales_payment_backup_v25_20260804 (
  id bigint primary key,
  row_data jsonb not null,
  backed_up_at timestamptz not null default now()
);

insert into public.telemetry_sales_payment_backup_v25_20260804 (id, row_data)
...
create table if not exists public.telemetry_stock_balance_backup_20260804
  (like public.stock_location_balances including defaults);

create unique index if not exists telemetry_stock_balance_backup_20260804_id_idx
  on public.telemetry_stock_balance_backup_20260804 (id);

insert into public.telemetry_stock_balance_backup_20260804
select balance.*
...
create table if not exists public.telemetry_coffee_container_backup_20260804
  (like public.machine_coffee_containers including defaults);

create unique index if not exists telemetry_coffee_container_backup_20260804_id_idx
  on public.telemetry_coffee_container_backup_20260804 (id);

insert into public.telemetry_coffee_container_backup_20260804
select container.*
...
with ingest_sequence as (
  ...
  where sequence.event_at >= timestamptz '2026-08-03 22:00:00+00'
    and sequence.event_at < timestamptz '2026-08-04 22:00:00+00'
