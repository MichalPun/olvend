...
begin
  ...
  loop
    ...
    v_allow_negative_source := coalesce((movement_row->>'allow_negative_source')::boolean, false);
    if v_from_location_id is not null then
      select v_allow_negative_source or exists (
        select 1
      ...
      if not v_allow_negative_source and (source_balance.id is null or coalesce(source_balance.quantity_on_hand, 0) + 0.0001 < v_quantity) then
        ...
        select sl.name into v_source_location_name from public.stock_locations sl where sl.id = v_from_location_id;
        raise exception 'Nedostatek zásoby: % na % (požadováno %, dostupné %)',
          coalesce(v_product_name, 'produkt #' || v_product_id::text),
          coalesce(v_source_location_name, 'skladové místo #' || v_from_location_id::text),
          v_quantity,
          coalesce(source_balance.quantity_on_hand, 0);
      end if;
