-- One-time guarded reconstruction for the 66 physical PA2 vends from the
-- 2026-08-06 payment repair that exceeded the already-zero book balance.
begin;
...
create table if not exists public.telemetry_stock_reconstruction_backup_v25_20260806 (
...
with original_unpaid as (
  select id
  from public.telemetry_sales_payment_backup_v25_20260806
...
begin
  ...
  from public.telemetry_stock_reconstruction_backup_v25_20260806;

  if not (
    v_shortage = 66
    or (v_shortage = 0 and v_existing_backup = 66)
  ) then
    raise exception 'Expected the guarded 66-unit IMA stock reconstruction, found % units with % already backed up.',
...
insert into public.telemetry_stock_reconstruction_backup_v25_20260806 (sale_event_id, row_data)
select
...
select
  ...
  '2026-08-06 · rekonstrukce chybějícího stavu podle fyzického PA2 výdeje #' || shortage.sale_event_id
from ima_stock_shortfalls shortage
where not exists (
  select 1
...
select
  ...
  '2026-08-06 · fyzický telemetrický výdej #' || shortage.sale_event_id || ' podle PA2 · rekonstruovaný stav'
from ima_stock_shortfalls shortage
where not exists (
  select 1
  from public.stock_movements_v13 movement
...
begin
  with expected as (
    ...
    from public.telemetry_stock_reconstruction_backup_v25_20260806 backup
  ...
  from public.stock_movements_v13 movement
  join public.telemetry_stock_reconstruction_backup_v25_20260806 backup
    on movement.reference_id = 'telemetry-stock-reconstruction:' || backup.sale_event_id
  where movement.reference_type = 'telemetry_stock_reconstruction';

  select coalesce(sum(quantity_base_units), 0) into v_reconstructed_out
  from public.stock_movements_v13 movement
  join public.telemetry_stock_reconstruction_backup_v25_20260806 backup
    on movement.reference_id = 'telemetry-sale-reconstruction:' || backup.sale_event_id
  where movement.reference_type = 'telemetry_sale_reconstruction';

  if v_missing <> 0 or v_reconstructed_in <> 66 or v_reconstructed_out <> 66 then
