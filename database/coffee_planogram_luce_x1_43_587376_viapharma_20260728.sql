-- Planogram [43] Luce X1 I (7)-2026-07-28.xlsx
-- Machine DB id 37, evidence 43, TID 587376, ViaPharma location_id 54.
-- Instant X1: no bean hopper; Z1 is white chocolate. Selection 0 is aggregate telemetry.
-- Source SKU 238 has no catalog recipe; button 14 is replaced with Kakaový nápoj Cream SKU 234.

do $$
declare
  v_machine_id bigint := 37;
begin
  ...
  set location_id = 54,
      ...
      note = concat_ws(
        ' ', nullif(note, ''),
        'Planogram 2026-07-28; Brno - ViaPharma; TID 587376; instantní X1; souhrnná telemetrie volbou 0.'
      )
  ...
  values
    (v_machine_id, 'IMA', '587376', true, 'TID 587376 pro Luce X1 I (7) / automat 43 / ViaPharma.'),
    (v_machine_id, 'GP', '587376', true, 'TID 587376 pro Luce X1 I (7) / automat 43 / ViaPharma.')
  ...
  select
    ...
    d.sort_order, true,
    'Import planogramu 43 / 2026-07-28.'
      || case
        when d.container_code = 'Z8'
          then ' Export uvádí kelímek po 100 ks, skladově doplňujeme po 50 ks.'
        when d.container_code = 'Z9'
          then ' Technologická voda.'
        else ''
      end
  from (values
    ('Z1','51', 2000::numeric, 2000::numeric,'g', 1000::numeric,'g',1),
    ('Z2','43', 3000::numeric, 2973::numeric,'g', 1500::numeric,'g',2),
    ('Z3','48', 2000::numeric, 1940::numeric,'g', 1000::numeric,'g',3),
    ('Z4','47', 2000::numeric, 1977::numeric,'g', 1000::numeric,'g',4),
    ('Z5','44', 1000::numeric,  986::numeric,'g',  500::numeric,'g',5),
    ('Z6','46', 2000::numeric, 1944::numeric,'g', 1000::numeric,'g',6),
    ('Z7','49', 3000::numeric, 2979::numeric,'g', 1000::numeric,'g',7),
    ('Z8','45',  400::numeric,  393::numeric,'ks',  50::numeric,'ks',8),
    ('Z9','214',20000::numeric,18740::numeric,'ml',1000::numeric,'ml',9)
  ) d(
  ...
  select
    ...
    case when d.selection_code = '14'
      then 'Zdrojové SKU 238 bez receptury je nahrazeno Kakaový nápoj Cream SKU 234.'
      else null
    end,
    d.last_counter, d.grid_column, d.grid_row_from_bottom, d.sort_order, true,
    'Import planogramu 43 / 2026-07-28.'
  from (values
    ('1', '239',12::numeric,  8::integer,1,4, 1),
    ('2', '239',12::numeric,107::integer,1,3, 2),
    ('3', '215',16::numeric, 68::integer,1,2, 3),
    ('4', '215',16::numeric,291::integer,1,1, 4),
    ('5', '215',16::numeric,177::integer,2,4, 5),
    ('6', '245',16::numeric,211::integer,2,3, 6),
    ('7', '222',16::numeric, 65::integer,2,2, 7),
    ('8', '245',16::numeric, 86::integer,2,1, 8),
    ('9', '242',16::numeric, 64::integer,3,4, 9),
    ('10','219',16::numeric, 75::integer,3,3,10),
    ('11','230',16::numeric, 79::integer,3,2,11),
    ('12','250',16::numeric, 54::integer,3,1,12),
    ('13','233',16::numeric, 38::integer,4,4,13),
    ('14','234',16::numeric,449::integer,4,3,14),
    ('15','236',16::numeric, 39::integer,4,2,15),
    ('16','217',16::numeric,146::integer,4,1,16)
  ) d(
  ...
  select
    ...
    'Fyzická X1 volba; planogram 2026-07-28 / TID 587376 / ViaPharma.'
  ...
  values (
    ...
    'Agregační X1 slot TID 587376; bez fyzického tlačítka a receptury.'
  )
  on conflict (machine_id, slot_code) do update
  set product_name = excluded.product_name,
      product_sku = excluded.product_sku,
  ...
  from (values
    ('1',8),('2',107),('3',68),('4',291),
    ('5',177),('6',211),('7',65),('8',86),
    ('9',64),('10',75),('11',79),('12',54),
    ('13',38),('14',449),('15',39),('16',146)
