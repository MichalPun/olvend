-- One-time guarded reconstruction for the 7 physical PA2 vends from the
-- 2026-08-03 payment repair that exceeded the already-zero book balance.
begin;
...
create table if not exists public.telemetry_stock_reconstruction_backup_v25_20260805_aug3 (
...
with original_unpaid as (
  select id
  from public.telemetry_sales_payment_backup_v25_20260803
...
begin
  ...
  from public.telemetry_stock_reconstruction_backup_v25_20260805_aug3;

  if not (
    v_shortage = 7
    or (v_shortage = 0 and v_existing_backup = 7)
  ) then
    raise exception 'Expected the guarded 7-unit IMA stock reconstruction, found % units with % already backed up.',
...
insert into public.telemetry_stock_reconstruction_backup_v25_20260805_aug3 (sale_event_id, row_data)
select
...
begin
  with expected as (
    ...
    from public.telemetry_stock_reconstruction_backup_v25_20260805_aug3 backup
  ...
  from public.stock_movements_v13 movement
  join public.telemetry_stock_reconstruction_backup_v25_20260805_aug3 backup
  ...
  if v_missing <> 0 or v_reconstructed_in <> 7 or v_reconstructed_out <> 7 then
