...
create or replace function public.ensure_post_route_exception_inventory_v31(p_route_plan_id bigint)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  with vehicle_balances as (
    ...
    where visit.route_plan_id = p_route_plan_id
      ...
      and (
        item.issue_type = 'missing_stock'
        or (
          coalesce(item.actual_add_quantity, 0) <= 0.0001
  ...
  insert into public.inventory_audits (
    ...
    null,
    'Cílená kontrola po trase #' || p_route_plan_id || ': operátor označil nedostupné zboží nebo automat po návštěvě zůstal na 0, přestože stejné zboží podle evidence zůstává ve vozidle. Počítají se pouze vypsané podezřelé položky.',
    ...
    select
      ...
      string_agg(
        distinct concat_ws(' · ',
          ...
          case
            when item.issue_type = 'missing_stock' then 'operátor: zboží není na vozidle'
            else 'stav 0, doplněno 0'
          end
        ),
        ' / '
      ) as reason_note
...
as $$
begin
  perform public.ensure_post_route_exception_inventory_v31(new.id);
  return new;
end
$$;

drop trigger if exists trg_post_route_exception_inventory_v30 on public.route_plans;
drop trigger if exists trg_post_route_exception_inventory_v31 on public.route_plans;
create trigger trg_post_route_exception_inventory_v31
