...
create table if not exists public.telemetry_sales_payment_backup_v25_20260806 (
  id bigint primary key,
  row_data jsonb not null,
  backed_up_at timestamptz not null default now()
);

insert into public.telemetry_sales_payment_backup_v25_20260806 (id, row_data)
...
create table if not exists public.telemetry_stock_balance_backup_20260806
  (like public.stock_location_balances including defaults);

create unique index if not exists telemetry_stock_balance_backup_20260806_id_idx
  on public.telemetry_stock_balance_backup_20260806 (id);

insert into public.telemetry_stock_balance_backup_20260806
select balance.*
...
create table if not exists public.telemetry_coffee_container_backup_20260806
  (like public.machine_coffee_containers including defaults);

create unique index if not exists telemetry_coffee_container_backup_20260806_id_idx
  on public.telemetry_coffee_container_backup_20260806 (id);

insert into public.telemetry_coffee_container_backup_20260806
select container.*
...
with ingest_sequence as (
  ...
  where sequence.event_at >= timestamptz '2026-08-05 22:00:00+00'
...
begin
  for sale in
    ...
    where event.provider = 'IMA'
      and event.source_event_at >= timestamptz '2026-08-05 22:00:00+00'
      and event.source_event_at < timestamptz '2026-08-06 22:00:00+00'
