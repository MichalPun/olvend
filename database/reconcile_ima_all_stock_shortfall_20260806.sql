-- Complete the physical stock audit for every IMA food vend already received
-- today before zero-balance reconstruction became automatic. Book balances stay
-- at zero; the exact shortfall is recorded as a balanced reconstruction and sale.
begin;

create table if not exists public.telemetry_all_stock_shortfall_backup_v26_20260806 (
  sale_event_id bigint primary key,
  row_data jsonb not null,
  backed_up_at timestamptz not null default now()
);

create temporary table ima_all_stock_shortfalls on commit drop as
with expected as (
  select
...
begin
  ...
  from ima_all_stock_shortfalls;

  select coalesce(sum((row_data ->> 'quantity')::numeric), 0) into v_existing_backup
  from public.telemetry_all_stock_shortfall_backup_v26_20260806;

  if not (
    v_shortage = 121
    or (v_shortage = 0 and v_existing_backup = 121)
  ) then
    raise exception 'Expected the guarded 121-unit all-day IMA stock reconstruction, found % units with % already backed up.',
      v_shortage, v_existing_backup;
  end if;

  -- Serialize only the affected product balances, not the whole live fleet.
  perform balance.id
  ...
  if v_book_balance <> 0 then
    raise exception 'All-day IMA reconstruction is only valid above a zero book balance; current guarded balance is %.',
      v_book_balance;
  end if;
end $$;

insert into public.telemetry_all_stock_shortfall_backup_v26_20260806 (sale_event_id, row_data)
select
...
from ima_all_stock_shortfalls shortage
...
begin
  ...
  loop
    perform public.apply_telemetry_stock_depletion(array[event_row.sale_event_id]);
  end loop;
end $$;
...
begin
  with expected as (
    ...
    from public.telemetry_all_stock_shortfall_backup_v26_20260806 backup
  ...
  if v_missing <> 0 then
    raise exception 'All-day IMA stock reconstruction validation failed: % units remain missing.', v_missing;
  end if;
end $$;
